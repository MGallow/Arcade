...
def construct_model():
    if loaded_model is not None:
        ...
        model_location = os.path.join(assets, loaded_model)
        model = model_from_json(open(model_location).read())
        if loaded_weights is not None:
            #load previous weights
            weights_location = os.path.join(assets, loaded_weights)
...
def train_model():
    ...
    while t < nb_steps:
        ...
        if done:
            ...
            print("********** EPISODE ENDED ***********")
            print("TOTAL REWARD:", total_reward, "/EPISODE AVERAGE:", np.mean(rewards))

            #train on minibatches. We only train after observation period is over so that there are sufficient number of frames to sample from
            if t > observe:
                #sample a minibatch to train on
                minibatch = random.sample(memory, batch)

                inputs = np.zeros((batch, x1.shape[1], x1.shape[2], x1.shape[3]))  #50, img_rows, img_cols, 1
